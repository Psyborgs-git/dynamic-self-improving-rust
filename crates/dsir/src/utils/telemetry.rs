...
const DEFAULT_PRETTY_FILTER: &str = "dsir=debug";
...
/// Installs process-global, pretty tracing output for dsir.
///
/// Behavior:
/// - Uses `RUST_LOG` when present.
/// - Falls back to `dsir=debug` when `RUST_LOG` is unset/invalid.
