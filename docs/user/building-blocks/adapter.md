...
An adapter sits between your [signature](../building-blocks/signature.md) and the LM. It formats typed inputs into prompts and parses LM responses back into typed outputs.
...
After parsing, the adapter runs [constraints](../building-blocks/constraints.md):
...
```rust
use dsir::ChatAdapter;

let adapter = ChatAdapter;

// See what prompt would be generated
let system = adapter.format_system_message_typed::()?;
let user = adapter.format_user_message_typed::(&input);
...
Here's what a prompt looks like for a complex nested type (from [`examples/16-insurance-claim-prompt.rs`](https://github.com/darinkishore/dsir/blob/main/crates/dsir/examples/16-insurance-claim-prompt.rs)):
