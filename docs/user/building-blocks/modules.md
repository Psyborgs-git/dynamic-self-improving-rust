...
Compose prompting strategies over any signature
...
A `Predict` calls the LM directly against your signature. A module adds behavior around that call — extra output fields, retry loops, tool use — without changing your signature definition.
...
ChainOfThought returns `WithReasoning`, not bare `QAOutput`. But you rarely write that type — inference handles it:
...
`WithReasoning` reads as English: "QA output, with reasoning."
...
Demos for ChainOfThought include reasoning — they're `Example>`. The reasoning field shows the LM what good chain-of-thought looks like.

```rust
use dsir::{Example, Augmented, Reasoning, WithReasoning};
...
```rust
use dsir::{Module, Predict, ChainOfThought, Predicted, PredictError, Signature};
...
`#[derive(facet::Facet)]` on the struct is what makes optimizer discovery work — the framework finds `retrieve` and `answer`'s inner predictor automatically without annotations. See [Optimization](../optimizers/overview.md) for details.
...
`.and_then()` for fallible transforms that return `Result`.
...
let results = dsir::forward_all(&cot, inputs, 10).await;
...
Changing the strategy may change the output type — `Predict` returns `QAOutput`, `ChainOfThought` returns `WithReasoning`. The compiler catches every downstream breakage. No runtime surprises.
...
|--------|-------------|-------------|-------------------|
| `Predict` | Direct LM call | `S::Output` | 1 (itself) |
| `ChainOfThought` | Reason then answer | `WithReasoning` | 1 |
