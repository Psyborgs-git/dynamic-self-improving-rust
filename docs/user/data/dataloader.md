...
No manual `RawExample -> Example` conversion is required.

## Core API

```rust
use dsir::{DataLoader, Example, Signature, TypedLoadOptions};
```
...
Mapper overloads:
- `DataLoader::load_json_with::<S, F>(...)`
- `DataLoader::load_csv_with::<S, F>(...)`
- `DataLoader::load_parquet_with::<S, F>(...)`
- `DataLoader::load_hf_with::<S, F>(...)`
...
```rust
use dsir::{DataLoader, Signature, TypedLoadOptions};
...
use dsir::{DataLoader, TypedLoadOptions, UnknownFieldPolicy};
...
```rust
use dsir::{DataLoader, Example, TypedLoadOptions};
