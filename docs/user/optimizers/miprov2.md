...
LLM-guided prompt generation with best practices
...
Use the shared data ingress guide: [`DataLoader`](../data/dataloader.md).
...
- Typed `MIPROv2::compile` is **instruction-only** on the static path (demo seeding from traces is a known gap).
- Use **`compile_dyn`** for runtime programs — this path seeds demos from successful traces.
- See [compile_dyn](compile-dyn.md) and [optimizers overview](overview.md).
